...
async function postinstall() {
    ...
    try {
        ...
        // Set URL in case the remote already exists
        await run('git', ['-C', 'vendor', 'remote', 'set-url', 'origin', packageJSON.osu.repository]);
    }
...
async function run(command, args) {
    ...
    if (exitCode !== 0) {
        throw new Error(`Command "${command} ${args.join(' ')}" exited with code ${exitCode}`);
    }
}
